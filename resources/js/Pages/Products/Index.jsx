...
export default function Index({ products, categories, filters, orgUnits, userUnit }) {
    ...
    // Sync products data when filters/search change (page 1) or append on pagination (page > 1)
    useEffect(() => {
        if (!products?.current_page || products.current_page === 1) {
            setItems(products?.data || []);
        } else {
            setItems((prev) => {
                const existingIds = new Set(prev.map((p) => p.id));
                const nextItems = (products?.data || []).filter((p) => !existingIds.has(p.id));
    ...
    // IntersectionObserver to automatically load more products on scroll
    useEffect(() => {
        if (!products?.next_page_url) return;
    ...
    }, [products?.next_page_url, loadingMore, loadMore]);
        ...
        <AuthenticatedLayout>
            ...
            {items.length === 0 ? (
                ...
                <>
                    ...
                    {/* Sentinel & Infinite Scroll State */}
                    <div className="mt-8 flex flex-col items-center justify-center space-y-3 pb-8">
                        {/* Intersection Observer Sentinel */}
                        <div ref={sentinelRef} className="h-4 w-full pointer-events-none" />

                        {/* Loading State Spinner */}
                        {loadingMore && (
                            <div className="flex items-center space-x-2.5 px-5 py-2.5 rounded-full bg-white shadow-sm border border-amber-200 text-amber-900 font-semibold text-sm animate-pulse">
                                <Loader2 className="w-5 h-5 text-amber-600 animate-spin" />
                                <span>{t('loading_more_products')}</span>
                            </div>
                        )}

                        {/* Manual Fallback Load More Button */}
                        {!loadingMore && products?.next_page_url && (
                            <button
                                type="button"
                                onClick={loadMore}
                                className="px-6 py-2.5 rounded-2xl bg-white hover:bg-amber-50 text-amber-800 border border-amber-200 shadow-sm font-bold text-sm transition-all hover:scale-[1.02] cursor-pointer"
                            >
                        ...
                        {!products?.next_page_url && items.length > 0 && (
                            <div className="text-center py-4 text-xs font-semibold text-gray-500 flex items-center space-x-2">
                                <span className="w-12 h-px bg-gray-300 inline-block" />
                                <span>✓ {t('viewed_all_products')} ({items.length})</span>
                                <span className="w-12 h-px bg-gray-300 inline-block" />
                            </div>
                        )}
                    </div>
